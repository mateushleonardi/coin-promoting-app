...
contract('NftMarket', (accounts) => {
  ...
  describe('Mint token', () => {
    ...
    it('owner of the first token should be addres[0]', async () => {
      const owner = await _contract.ownerOf(1);
      assert.equal(
        owner,
        accounts[0],
        'Owner of token in not matching address[0]',
      );
    });

    it('first token should point to the correct tokenURI', async () => {
      const actualTokenURI = await _contract.tokenURI(1);
      assert.equal(actualTokenURI, tokenURI, 'tokenURI is not correcttly set');
    });
